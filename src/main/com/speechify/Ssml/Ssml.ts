...
function createSSMLNode(tag: string | null = null, text: string | null = null): SSMLNode {
  return {
    tag,
    text,
    attributes: new Map(),
...
function parseSSML(ssml: string): SSMLNode {
  ...
  if (speakOpenEnd === -1) throw new Error('Tags could not be parsed');
  const speakAttrs = trimmed.substring(6, speakOpenEnd).trim();
  const speakNode = createSSMLNode('speak');
  if (speakAttrs) parseAttributes(speakAttrs, speakNode.attributes);
...
function parseContentStack(content: string, parentNode: SSMLNode) {
  let i = 0;
  let textBuffer = '';
  while (i < content.length) {
    if (content[i] === '<') {
      ...
      if (closeBracket === -1) throw new Error('Tags could not be parsed');
      ...
      if (!tagName) throw new Error('Tags could not be parsed');

      if (isSelfClosing) {
        // Self-closing tag
        const node = createSSMLNode(tagName);
        if (spaceIdx !== -1) parseAttributes(tagContent.substring(spaceIdx + 1), node.attributes);
      ...
      while (depth > 0) {
        ...
        if (nextClose === -1) throw new Error('Tags could not be parsed');
...
function parseElementStack(input: string): SSMLNode {
  if (!input.startsWith('<') || !input.endsWith('>')) throw new Error('Tags could not be parsed');
  const firstClose = input.indexOf('>');
  if (firstClose === -1) throw new Error('Tags could not be parsed');
  ...
  if (closeIdx === -1) throw new Error('Tags could not be parsed');
  const inner = input.substring(firstClose + 1, closeIdx);
  if (inner.length > 0) parseContentStack(inner, node);
  return node;
}

function parseAttributes(attrString: string, attributes: Map<string, string>) {
  let i = 0;
  while (i < attrString.length) {
    ...
    if (i === nameStart) throw new Error('Attributes could not be parsed');
    const name = attrString.substring(nameStart, i);
    while (i < attrString.length && /\s/.test(attrString[i])) i++;
    if (i >= attrString.length || attrString[i] !== '=') throw new Error('Attributes could not be parsed');
    i++;
    while (i < attrString.length && /\s/.test(attrString[i])) i++;
    if (i >= attrString.length || (attrString[i] !== '"' && attrString[i] !== "'")) throw new Error('Attributes could not be parsed');
    const quote = attrString[i];
    i++;
    const valueStart = i;
    while (i < attrString.length && attrString[i] !== quote) i++;
    if (i >= attrString.length) throw new Error('Attributes could not be parsed');
...
function ssmlNodeToText(node: SSMLNode | null): string {
  if (!node) return '';
  if (node.text !== null) return node.text;
  return node.children.map(ssmlNodeToText).join('');
}

export { parseSSML, ssmlNodeToText, SSMLNode };